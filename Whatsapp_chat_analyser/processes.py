...
def chat_contri(df):
    ...
    ax1.pie(
        count, labels=name, colors=colors, autopct="%1.1f%%", shadow=True, startangle=90
    )
    ax1.axis("equal")
...
def monthly_timeline(selected, df):
    ...
    monthly_timeline = (
        df.groupby(["year", "month_num", "months"]).count()["messages"].reset_index()
    )
    time = []
    for i in range(monthly_timeline.shape[0]):
        time.append(
            monthly_timeline["months"][i] + "-" + str(monthly_timeline["year"][i])
        )
...
# AFTER SOME CORRECTION TO ADAPT FOR BOTH IOS AND ANDROID


def preprocess(data):
    # Updated pattern to accommodate both iOS and Android formats
    pattern = r"\d{1,2}\/\d{1,2}\/\d{2,4},\s\d{1,2}:\d{2}\s(?:[AaPp][Mm])?\s-\s|\[\d{1,2}\/\d{1,2}\/\d{2,4},\s\d{1,2}:\d{2}:\d{2}\s(?:[AaPp][Mm])\]"
    ...
    # Try parsing with the common formats for both iOS and Android
    for fmt in (
        "%d/%m/%y, %I:%M:%S %p",
        "%d/%m/%y, %I:%M %p - ",
        "%d/%m/%Y, %I:%M %p - ",
        ...
        try:
            df["dates"] = pd.to_datetime(df["dates"], format=fmt)
            break
        except ValueError:
            continue
    else:
